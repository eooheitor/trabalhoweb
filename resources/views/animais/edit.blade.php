        ...
        <form method="POST" action="{{ route('animais.update', $animal->id) }}" enctype="multipart/form-data">
            ...
            <div class="mb-6">
                ...
                <select name="tipo" id="tipo"
                    ...
                    <option value="C" {{ old('tipo', $animal->tipo) == 'C' ? 'selected' : '' }}>Cachorro</option>
                    <option value="G" {{ old('tipo', $animal->tipo) == 'G' ? 'selected' : '' }}>Gato</option>
                </select>
                @error('tipo')
                    <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-6">
                ...
                <textarea id="descricao" name="descricao" rows="4"
                    ...
                    placeholder="Descreva o animal">{{ old('descricao', $animal->descricao) }}</textarea>
                @error('descricao')
                    <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                <label for="dataresgate" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Data
                    Resgate</label>
                <input type="date" id="dataresgate" name="dataresgate"
                    value="{{ old('dataresgate', $animal->dataresgate) }}"
                    class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" />
                @error('dataresgate')
                    <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-6">
                ...
                <select name="castrado" id="castrado"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    <option value="" selected>Selecione</option>
                    <option value="0" {{ old('castrado', $animal->castrado) == '0' ? 'selected' : '' }}>Não
                    </option>
                    <option value="1" {{ old('castrado', $animal->castrado) == '1' ? 'selected' : '' }}>Sim
                    </option>
                </select>
                @error('castrado')
                    <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
            ...
            <div class="mb-6">
                ...
                @if ($animal->foto)
                    <div class="mt-4 mb-4">
                        <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Foto
                            Atual</label>
            ...
            <button type="submit"
                class="text-white bg-black hover:bg-gray-900 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-black dark:hover:bg-gray-900 dark:focus:ring-blue-800">Salvar</button>
        </form>
    </div>
</x-app-layout>
